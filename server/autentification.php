<?php
	session_start();
	require_once './HMVC/usersMVC/dispatcherMVC.php';
	require_once './HMVC/usersMVC/simpleUserMVC.php';
	require_once './HMVC/usersMVC/guestMVC.php';
	function preprocessingOfLoginAndPass ($_login, $_pass) {
		...
		return 'OK';//$statusOfPreprocessing; // "OK" and "notOK"
	}
	function checkLoginAndPassInDB ($_login, $_pass) {	
		//search in DB user with login == $login and pass == md5($pass). 
		return 'simpleUser';//$statusOfUser; // status from DB record of user: 1.dispatcher 2.simpleUser 3.guest 4.invalid
	}
	$login = $_POST['login'];
	$pass = $_POST['pass'];
	if (preprocessingOfLoginAndPass($login, $pass) == "notOK") {
		...
		if ($statusOfUser == 'invalid') {
			...
			switch ($statusOfUser) {
				case 'dispatcher':	$_SESSION['controller'] = new ControllerOfDispatcher;
									break;
				case 'simpleUser':	$_SESSION['controller'] = new ControllerOfSimpleUser;
									break;
				case 'guest':		$_SESSION['controller'] = new ControllerOfGuest;
									break;
				default:			session_destroy();
									echo 'Who is this guy?'; // !!!!! same thing....?????????
									break;
			}
			echo $_SESSION['controller']->view->createUserInterfaceInHtml();
		}
	}
?>